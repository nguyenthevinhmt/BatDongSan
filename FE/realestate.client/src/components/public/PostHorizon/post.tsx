            ...
            <div style={{ width: "720px", height: '150px', marginBottom: '60px' }}>
                <Flex>
                    <Link href={`/home/post/detail/${data?.id}`} style={{ flex: 3 }}>
                        <Image src={data?.firstImageUrl} alt="#" width={200} height={150} />
                    </Link>
                    <div style={{ flex: 7 }}>
                        <Link href={`/home/post/detail/${data?.id}`}><h3 style={{ color: '#444', fontWeight: '500' }}>{data?.title}</h3></Link>
                        <p style={{ margin: "10px 0px", color: 'rgba(0, 0,0, 0.6)' }}>{`${data?.district}. ${data?.province}`}</p>
                        <Flex align='center' style={{ width: '140px' }}>
                            <div>{data?.area} m²</div>
                        </Flex>
                        <Flex justify='space-between' style={{ marginTop: "40px" }}>
                            <div style={{ color: 'rgba(0, 0,0, 0.6)' }}>{formatDate(dayjs(data?.createdDate))}</div>
                            <Flex>
                                <Flex style={{ fontSize: '18px', fontWeight: '600', color: "#444", marginRight: '10px' }}>
                                    <div style={{ marginRight: '10px' }}>{formatCurrency(data?.price)}</div>
                                    <div>{data?.area} m²</div>
                                </Flex>
