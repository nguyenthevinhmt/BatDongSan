...
export default async function handler(
    ...
    try{
      ...
      const accessTokenCookie = `access_token=${data.access_token}; HttpOnly; SameSite=Lax; Expires=${new Date(data.expires_in).toUTCString()}`;
      const refreshTokenCookie = `refresh_token=${data.refresh_token}; HttpOnly; SameSite=Lax; Expires=${new Date(data.expires_in).toUTCString()}`;
