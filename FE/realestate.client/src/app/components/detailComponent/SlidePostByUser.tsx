...
const SlidePostByUser: React.FC<SlidePostByUserProps> = ({
  userId,
  postType,
}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (postType === 1) {
        const response = await getSalePostByAuthor(
          { pageSize: -1, postType: postType },
          userId
        );
        ...
        const response = await getRentPostByAuthor(
          { pageSize: -1, postType: postType },
          userId
        );
        const posts: IPost[] = response?.data?.items?.map((post: any) => ({
        ...
        <>
          <Flex justify="space-between" style={{ paddingLeft: 5 }}>
            <Flex align="flex-end" style={{ width: "100%" }}>
              ...
              <Button
                onClick={onClickNext}
                disabled={indexPost >= listPost?.length - 5}
          ...
          <Flex
            justify="flex-start"
            style={{ width: "100%", paddingLeft: 5, paddingTop: 5 }}
          >
            {listPost && listPost?.length >= indexPost + 5
              ? listPost
                  ?.slice(indexPost, indexPost + 5)
                  ?.map((item, index) => {
                    return (
                       <div style={{ marginRight: 10 }} key={item.id}>
                          <PostCard
                            data={item}
                            option={item.options}
                            loading={false}
