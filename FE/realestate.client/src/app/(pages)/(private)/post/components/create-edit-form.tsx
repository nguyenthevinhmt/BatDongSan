...
const CreateEditForm = ({ type }: { type: number }) => {
	...
	useEffect(() => {
		const fetchLocation = async () => {
			const res = await axios.get(`http://dev.virtualearth.net/REST/v1/Locations?q=${encodeURIComponent(location?.street + " " + location.wards + " " + location.districts + " " + location.provinces)}&key=${environment.BingMapsApiKey}`)
			console.log(" location", res.data.resourceSets[0].resources[0].point.coordinates[0])
			console.log(" location", res.data.resourceSets[0].resources[0].point.coordinates[1])
			const coordinates = {
			...
			>
				...
				>
					...
					<Flex justify="center" gap={"small"}>
						...
						>
							<Input value={location.street} onChange={(e) => {
								setLocation((prev: any) => {
									return {
										...prev,
					...
					</Form.Item>
					{
						showMap && <Flex justify="center">
							<BingMapsReact
								...
								pushPins={[{
									center: {
										latitude: 20.99120903,
										longitude: 105.86445618,
									}
								}]}
								viewOptions={{
									center: { latitude: 20.99120903, longitude: 105.86445618 },
									mapTypeId: "road",
									zoom: 16,
								}}
